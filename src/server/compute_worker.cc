...
#include "server/compute_worker.h"
...
DEFINE_string(config, AGE_ROOT "/config/conf.ini.compute", "config path");
...
DEFINE_string(schema, AGE_ROOT "/config/schema", "schema path");
...
int main(int argc, char *argv[]) {
    ...
    // Load nodes.
    Nodes compute_nodes(FLAGS_worker, ClusterRole::COMPUTE, FLAGS_rank);

    ComputeWorker worker(compute_nodes.getLocalNode(), FLAGS_schema, FLAGS_index);
    worker.start();
