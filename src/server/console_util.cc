...
void ConsoleUtil::ResetColor() { fputs("\x1B[0m", stdout); }
...
string ConsoleUtil::FetchConsoleResult() {
    ...
    if (history_append_dedup_) {
        ...
        if (cur_line_no_ != 0 && BUFFER_LINE != 0) {
            ...
            if (line_bfs_[history_line_pos] == s) copy_into_history_ = false;
        }
    }
...
void ConsoleUtil::OnPrintableKey(int key) {
    ...
    if (cur_line_len_ == BUFFER_SIZE) return;

    if (cur_line_pos_ == cur_line_len_) {
        // native print and append
        printf("%c", key);
        buffer_[cur_line_len_++] = key;
        cur_line_pos_++;
    } else {
...
void ConsoleUtil::OnKeyDelete() {
    ...
    if (cur_line_pos_ == cur_line_len_ || cur_line_len_ == 0) return;
...
void ConsoleUtil::WriteConsoleHistory(string path) {
    ...
    if (cur_line_no_ < BUFFER_LINE - 1) line_to_write = cur_line_no_;

    if (cur_line_no_ == 0) return;
...
void ConsoleUtil::SetOnQuitWrite(string path) {
    if (path.size() > 0) on_quit_write_ = true;
